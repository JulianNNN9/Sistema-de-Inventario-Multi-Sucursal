...
export function useTransferenciasList({ page = 0, size = 20, estado, branchId }: Options = {}) {
  ...
  useEffect(() => {
    let active = true;
    setLoading(true);
    listTransfers({ page, size, estado, branchId })
      .then((res) => {
        if (active) {
          setData(res);
          setError(null);
  ...
  }, [page, size, estado, branchId, reloadTick]);
